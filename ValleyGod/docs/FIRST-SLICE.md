...
## Graphics goal
Aim for a photoreal UE5 look (Lumen, Nanite, volumetric fog, wet dirt). Prefer **MetaHuman Creator Blueprints for all eight starting adults** plus **Quixel/FAB/PN_GrassLibrary** grass, trees, and dirt when those downloads are in `Content/`. Until then the project keeps procedural meshes and baked/runtime `M_*` materials so PLAY.bat still runs. See `METAHUMAN-QUIXEL.md`.
