...
- Eight tribes on a miniature Earth (one sculpted valley, seven placeholder continents, ocean between). Each tribe has one teacher and three adult learners. They walk, talk (English), hunt, cook/eat, sleep, teach, and build. Dialogue pools are per context and do not repeat back to back. No child bodies. Pairing is 21+ only.
...
- Intimacy scenes. Births in this pass are an abstract adult-only hook (new person arrives at 21). No child characters.
...
## Graphics goal
Aim for a photoreal UE5 look (Lumen, Nanite, volumetric fog, wet dirt). Milestone 1 prefers **MetaHuman Creator (Mara)** plus **Quixel/FAB/PN_GrassLibrary** grass, trees, and dirt when those downloads are in `Content/`. Until then the project keeps procedural meshes and baked/runtime `M_*` materials so PLAY.bat still runs. See `METAHUMAN-QUIXEL.md`.
