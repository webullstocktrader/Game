...
## Graphics goal
Aim for grounded UE5 look (Lumen, Nanite where useful, wet dirt, soft night). Prefer **MetaHuman Creator Blueprints for all eight starting adults** plus **Quixel/FAB/PN_GrassLibrary** grass, trees, and dirt when those downloads are in `Content/`. Until then the project keeps procedural meshes and baked/runtime `M_*` materials so PLAY.bat still runs. See `METAHUMAN-QUIXEL.md`.
