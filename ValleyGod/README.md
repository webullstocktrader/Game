...
7. Watch a chopped tree leave the world as wood, and a shelter go up as a stone, log, and thatch kit: site → frame → walls → roof. A baby grows up. Tech starts at Fire and tools and climbs toward Computing as the teacher’s camp learns. People prefer a MetaHuman or an equivalent photoreal pack; capsule bodies are only the empty-folder fallback. See `docs/PROGRESSION-LOCK.md`.
