...
void AValleyHUD::DrawHUD()
{
	...
	if (Sim.TribeCount > 0)
	{
		...
		ShadowText(PanelX + 16.f, 204.f, FString::Printf(TEXT("Willow know   %d"), FMath::RoundToInt(Home.Knowledge)), FLinearColor(0.7f, 0.72f, 0.65f), 0.8f);
	}
	...
	if (Pin >= 0 && Pin < Sim.VillagerCount)
	{
		...
		DrawBar(W - 360.f, H - 210.f, 340.f, 192.f, FLinearColor(0.02f, 0.03f, 0.02f, 0.62f));
		...
		ShadowText(W - 344.f, H - 128.f, FString::Printf(TEXT("%s"), UTF8_TO_TCHAR(vg::ActivityName(V.Current))), FLinearColor(0.9f, 0.88f, 0.78f), 0.85f);
		if (Tribe)
		{
			ShadowText(W - 344.f, H - 106.f,
