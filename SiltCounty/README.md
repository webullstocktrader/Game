...
Quixel / Megascans / Fab kits are not bundled. The first time the editor opens, it generates wet-ground, floodwater, truck paint, beacon, rain, the town materials (`M_TownBrick`, `M_TownClapboard`, `M_ConcreteBlock`, `M_MunicipalPaint`), and the shop materials (`M_ShopFloor`, `M_ShopWall`) under `Content/SiltCounty/Materials`. Lumen and Nanite are turned on in the project. The runtime terrain is a procedural mesh, so it is not Nanite; Lumen still lights the wet surfaces, water, and rain.
